...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  return response
    .status(403)
    .json({ error: "User limit exceeded , please subscribe" });
}
...
function checksTodoExists(request, response, next) {
  ...
  if (!validate(idTodo)) {
    return response.status(400).json({ error: "Todo not validate" });
  }

  const todoExists = user.todos.find((todo) => todo.id === idTodo);
  if (!todoExists) {
